...
def get_pending_orders(company):
    ...
    pending_orders = pd.read_csv(Locations.PENDING_ORDERS) if \
                        os.path.isfile( os.path.join(Locations.PENDING_ORDERS) ) else \
...
def get_approved_orders(company):
    ...
    approved_orders = pd.read_csv(Locations.APPROVED_ORDERS) if \
                        os.path.isfile( os.path.join(Locations.APPROVED_ORDERS) ) else \
...
def get_approved_buffer_orders(company):
    ...
    approved_buffer_orders = pd.read_csv(Locations.APPROVED_BUFFER_ORDERS) if \
                        os.path.isfile( os.path.join(Locations.APPROVED_BUFFER_ORDERS) ) else \
                        pd.DataFrame(columns=Keywords.EXECUTED_ORDER_COLUMS)
...
def get_squared_orders(company):
    ...
    squared_orders = pd.read_csv(Locations.SQUARED_ORDERS) if \
                        os.path.isfile( os.path.join(Locations.SQUARED_ORDERS) ) else \
                        pd.DataFrame(columns=Keywords.EXECUTED_ORDER_COLUMS)
...
def delete_pending_redundant_orders(stock_variables, company, ltp):
    ...
    orders_to_retain = pending_orders_company[(pending_orders_company[Keywords.PRICE] >= lower_thresh) & 
                                                (pending_orders_company[Keywords.PRICE] <= upper_thresh)]
    higher_than_ltp = orders_to_retain[orders_to_retain[Keywords.PRICE] >= ltp].sort_values(Keywords.PRICE, ascending = True).iloc[0]
    lower_than_ltp = orders_to_retain[orders_to_retain[Keywords.PRICE] < ltp].sort_values(Keywords.PRICE, ascending = False).iloc[0]
...
def place_new_order(stock_variables, company, ltp, order_type, price = -1, quantity = -1):
    ...
    pending_orders = pd.concat( (pending_orders,pd.DataFrame([ [company, order_id, price, quantity,
